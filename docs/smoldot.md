...
|---|---|---|
| Protocol iframe (`host.localhost`, production `dot.li`) | Domain resolution (Asset Hub query → CID), bitswap content fetching, Bulletin Paseo preimage submission | `apps/protocol/src/main.ts` (direct/shared-worker submodes) and `apps/protocol/src/protocol-shared-worker.ts` |
