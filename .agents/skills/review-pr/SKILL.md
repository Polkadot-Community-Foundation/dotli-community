...
Read changed files against the repo-root `AGENTS.md` architecture rules and any scoped `AGENTS.md` files inside touched subtrees.
...
bun run lint 2>&1 | grep -Ei "error" | head -30
bun run format:check 2>&1 | tail -10
```

For touched packages, run targeted vitest:

```sh
bun --filter @dotli/<pkg> run test
```

For host-touching changes, run the relevant Playwright spec:

```sh
cd apps/host && bunx playwright test tests/<file>.spec.ts --grep "<pattern>"
```
...
For dotli-specific surfaces, check:

- **Chain access**: only `apps/protocol/**` should import `@dotli/resolver/{smoldot,bulletin,chains,resolve}`. Consumer packages reach chains through `@dotli/protocol/client` only.
- **Metrics**: any new tag, span, or breadcrumb must come from `packages/metrics/src/spans.ts`. No string literals at call sites.
- **postMessage envelopes**: changes to `packages/protocol/src/messages.ts` are additive only. Handler updates required in BOTH `apps/protocol/src/main.ts` and `apps/protocol/src/protocol-shared-worker.ts` for shared-worker mode.
- **Config contract**: per-env values, contract addresses, endpoint URLs belong in `packages/config`, not inlined.
- **Resolver scope**: `packages/resolver` is read-only by convention. Signing belongs in `packages/auth`.
